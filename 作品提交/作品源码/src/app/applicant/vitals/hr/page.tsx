...
import { ChevronDown, ChevronUp, Calendar, Check, X, Heart, RefreshCw } from 'lucide-react';
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900">心率</h1>
            <p className="text-sm text-gray-500">HR (Heart Rate)</p>
          </div>
        </div>
        {/* 刷新按钮 */}
        <button
          ...
          title="刷新数据"
        >
          <RefreshCw className={`w-5 h-5 text-red-600 ${dataLoading ? 'animate-spin' : ''}`} />
        </button>
      </div>
        ...
        <CardContent className="p-4">
          ...
          <div className="flex gap-2 mb-4">
            <Button
              ...
              className={`flex-1 ${timeFilter === 'day' ? 'bg-red-500 hover:bg-red-600' : ''}`}
            >
            ...
            <Button
              ...
              className={`flex-1 ${timeFilter === 'week' ? 'bg-red-500 hover:bg-red-600' : ''}`}
            >
              周
            </Button>
            <Button
              ...
              className={`flex-1 ${timeFilter === 'month' ? 'bg-red-500 hover:bg-red-600' : ''}`}
            >
              月
            </Button>
          </div>
              ...
              {dayExpanded && (
                <div className="space-y-1 p-2 bg-gray-50 rounded-lg border border-gray-200">
                  {dayDataList.map((day, index) => (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <span className={`font-medium ${selectedDay === index ? 'text-red-600' : 'text-gray-700'}`}>
                          {day.label}
                        </span>
      ...
      <Card className="bg-gradient-to-r from-red-50 to-pink-50 border-red-200">
        <CardContent className="p-4">
          <h3 className="font-semibold text-red-800 mb-2">💡 健康科普</h3>
          <p className="text-sm text-red-700">
            正常成人安静时心率为60-100次/分。运动时心率会加快以满足身体供氧需求。
            持续高心率可能提示心脏负担加重，建议及时休息。当心率≥180bpm时系统会自动报警。
          </p>
        </CardContent>
      </Card>
    </div>
